...
- **`neverAutoSuggest` blocks AMBIENT auto-suggest; explicit per-question/section tags override it.** The flag (e.g. on `fears_ranking`) protects against a material appearing *incidentally* in contexts where it could feel jarring — it is **not** "this material has no relevance anywhere." An explicit `supplementaryDocumentRelevance` tag for a *specific* question/section is a deliberate, contextually-appropriate inclusion that **overrides** the flag: the user is engaging that question on purpose (e.g. Fears at advance-directive **q5** — "what I worry about when I think about death"). So the rule in **both** docs' section-tier loops is `if (activityMeta?.neverAutoSuggest && !relevance) continue` — skip only when flagged **and** untagged-for-this-question; otherwise tier by the tag. The **flat (no-section) overview has no question context — it IS the ambient context** — so it keeps `if (neverAutoSuggest) continue` unconditionally (a flagged material never shows there). This is symmetric: any future `neverAutoSuggest` material that gets an explicit tag is honored the same way. When you add a new auto-suggest surface, **grep `neverAutoSuggest`** and decide per-surface whether it's ambient (skip) or question-scoped (allow tag override).
