...
- **Password change was refactored to the same modal pattern** in this phase (Password row → "Change" → modal) for parity with the Email/Recovery rows. **Underlying logic is unchanged** — still client `signInWithPassword` + `updateUser({password})`; only the surface moved from an always-inline form to a modal.
- **Supabase "Secure password change" is intentionally OFF.** It requires its *own* OTP reauth (`reauthenticate()` nonce) that a fresh `signInWithPassword` does **not** satisfy, so with it ON, client `updateUser({password})` rejects with *"Current password required when setting new password"* — i.e. it **breaks** the client password change. Our security model instead is **app-level**: the modal requires the current password and we verify it via `signInWithPassword` before `updateUser` (defeats the session-only attacker), backed by Phase 5a's **password-changed notification to both primary + recovery** (detection/response). **Reversible** — if abuse patterns emerge post-launch, turn Secure-change back on and implement the OTP reauth (Option B). Tracked as an accepted risk (see memory `project-secure-password-change-off`).
...
**Notification policy (Phase 5 — mostly gap-filling).** Supabase now auto-notifies the **primary** email for password change / email change (old address) / sign-in-method & MFA events. We add the **recovery-email** side where policy says both, plus full notifications for events Supabase doesn't cover (account deletion, plan export). Password-change notification = keep client `updateUser` + a **thin notify endpoint** for the **recovery-email** side (primary side is Supabase's own password-changed notification — an **independent** toggle, unrelated to "Secure password change", which is OFF; see above). Do **not** move password update to a service-role admin write (loses the primary notification). plan-export = a thin endpoint the client calls post-export (PDF stays client-side). **Correction to the original Phase 5a framing:** it assumed client `updateUser` *preserves Supabase's secure-change re-auth* — that was wrong (secure-change breaks client `updateUser` without OTP). The notification half stands; the secure-change-re-auth half does not.
