...
- **Module split.** Account-**security** events (password, export, **deletion**, **recovery verify/remove**) live in `lib/account-notifications.ts`; **LC account-holder** events live in `lib/legacy-contact-notifications.ts`. All new account-holder bodies use `brandedEmail()` (which supplies the logo + "With care, The Nightside" sign-off — bodies must omit it).
- **Email change → old address is Supabase-handled** (its own auto notification, toggle ON); **we do not extend it.**
