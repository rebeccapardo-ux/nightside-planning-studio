...
- **Full plan export (shipped).** Both → primary + recovery. **JSON** route (`/api/plan/export-json`) notifies inline before returning. **Client-side PDF** uses a **thin `/api/plan/export-recorded` endpoint** the export page calls **fire-and-forget** after generating a *full* PDF. The **summary PDF does NOT notify** (lighter artifact; copy says "full export"). The thin-endpoint pattern is the way to bolt a server-side concern (notification, audit) onto a client-only flow.
- **Email change → old address is Supabase-handled** (its own auto notification, toggle ON); **we do not extend it.**
- **Still TODO:** account-deletion notifications (tied to the deferred 7-day soft-delete), and MFA / sign-in-method → recovery (TOTP phase).
