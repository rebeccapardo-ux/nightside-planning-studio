...
**Domain pages have TWO independent auto-surfacing systems — not one.** A domain row surfaces (a) **notes** via `allowedReflectPromptIds` (→ `notes.prompt_id`, `origin_type='prompt'` only) and (b) **materials** (the user's activity outputs / documents) via `relatedActivities` / `relatedDocumentTypes` (→ `entries.activity` / `entries.document_type`). These share no code (`resolveRowNotes` vs `itemEntries`/`ItemMaterials` in `app/app/domains/[domainId]/page.tsx`). **Assuming `allowedReflectPromptIds` is the only lever is the most likely domain-page regression** — it silently misses the entire materials half (e.g. Values/Fears Ranking under Healthcare `values_care_priorities`, Legacy Map under Legacy `life_story_shaped`, Important Contacts / Financial Information docs under Wills & Estates). A third affordance, `staticLinks`, is content-independent navigation, not surfacing.

**Domain pages do NOT have tier-based surfacing or fall-through** (in contrast with the wishes panels above). Surfacing is **binary** — an item is deemed relevant to a row by an explicit tag entry or it isn't; there is no Recommended/Also-relevant tiering and no tier-3 default. Reflection (`origin_type='reflection'`) and freeform notes **never** auto-surface on domain pages; an orphan prompt note (its `prompt_id` on no row) appears only on the Plan grid. Full detail + the "Default behavior" section in `docs/domain-page-placement-reference.md`.

> **⚠️ Redesign in progress — Pass 1 landed, Pass 2 pending.** The domain page is being simplified: the **Reflection + Learning** section is being removed from the UI, so the `orientation` rows in `lib/domain-structure.ts` become **back-end-only** — they keep driving tagging (`allowedReflectPromptIds` for prompt-note surfacing, `relatedActivities` for materials) but are **not rendered** as cards. Auto-surfacing moves **per-row → per-domain**: a prompt note surfaces in a domain if its `prompt_id` is in the union of that domain's rows' `allowedReflectPromptIds` (`getDomainPromptIds()` in `lib/domain-structure.ts`). The planning-status **segment model moves per-row → per-checkbox** (each readiness checkbox is one segment). The **single source of truth for the status label is now `qualitativeLabel()` in `lib/domain-status.ts`** (Option β: 0→Not yet started, <50%→Just beginning, <100%→Well underway, 100%→Complete); in Pass 2 it **replaces** the three drifted copies (count-based in `app/app/domains/[domainId]/page.tsx`, %-based in `app/components/DomainStateCard.tsx` + `lib/pdf/buildPlanData.ts`). Pass 1 added `getDomainPromptIds()` + `lib/domain-status.ts` as **uncalled** helpers and deactivated the domain tour; Pass 2 wires them and drops `domain_topic_notes` / `hidden_row_notes` / `domain_state.orient`. *(When Pass 2 lands, evolve this note to describe the permanent post-redesign state — do not delete it; it is the regression-prevention anchor.)*
