...
export default function DomainPlanningButton({ href, label, marginTop = 28 }: { href: string; label: string; marginTop?: number }) {
  return (
    <Link
      href={href}
      className="hover:opacity-90 transition-opacity"
      style={{
        display: 'inline-flex', alignItems: 'center', gap: 10, marginTop,
        background: '#BBABF4', color: '#130426', fontFamily: apfel, fontSize: 20, fontWeight: 600,
        padding: '18px 34px', borderRadius: 999, textDecoration: 'none', whiteSpace: 'nowrap',
      }}
    >
      {label} →
    </Link>
