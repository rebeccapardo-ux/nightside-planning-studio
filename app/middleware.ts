...
export async function middleware(request: NextRequest) {
  ...
  // Build the response first so cookies can be written onto it.
  ...
  if (ALWAYS_PUBLIC_PREFIXES.some(p => pathname.startsWith(p))) {
    ...
    if (user && pathname.startsWith('/auth/') && !pathname.startsWith('/auth/signup/payment') && !pathname.startsWith('/auth/signup/success') && !pathname.startsWith('/auth/signup/cancel')) {
